...
inline auto GetLine(std::span<char> buffer, char eol_indicator = '\n', uint32_t timeout_us = 50, bool echo = false) -> std::string_view {
  ...
  while (true) {
    ...
    if (c == PICO_ERROR_TIMEOUT || c == eol_indicator) [[likely]]  // The end of the line is either EOL or the user provided delimiter
      break;
    else [[unlikely]]  // If the end of the line hasn't been reached, place the data in the buffer
      buffer[end_index++] = c;
